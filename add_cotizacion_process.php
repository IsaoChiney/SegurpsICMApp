...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Obtener el ID del asesor y el nombre del asesor desde la sesión
    $asesor = $_SESSION['id'];
    ...
    if ($stmt = $mysqli->prepare($sql)) {
        ...
        if ($stmt->execute()) {
            // Si la cotización se guarda correctamente, redirigir al usuario después de registrar la cotización
            header("location: ../views/welcome.php");  // Redirigir al usuario a la página de bienvenida
            exit;
        } else {
